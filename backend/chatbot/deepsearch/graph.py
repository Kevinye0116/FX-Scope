...
async def search_web(state: SectionState):
    ...
    query_list = [query.search_query for query in search_queries if query.search_query and query.search_query.strip()]
    
    # 确保有有效的查询
    if not query_list:
        return {"source_str": "未找到有效的搜索查询。", "search_iterations": state["search_iterations"] + 1}
    
    # 添加错误处理
    search_results = []
    for query in query_list:
        try:
            if query and query.strip():  # 确保查询不为空
                results = DDGS(proxy=None).text(query, max_results=10)
    ...
    if not search_results:
        return {"source_str": "搜索过程中出现错误或未找到相关结果。", "search_iterations": state["search_iterations"] + 1}
...
def write_section(state: SectionState) -> Command[Literal[END, "search_web"]]:
    ...
    try:
        ...
        # 更严格的检查和错误处理
        if 'follow_up_queries' in tool_call:
            # 处理列表中的字符串格式
            if tool_call['follow_up_queries'] and isinstance(tool_call['follow_up_queries'], list):
                valid_queries = []
                for item in tool_call['follow_up_queries']:
                    if isinstance(item, str) and item.strip() and item != ",":
                        valid_queries.append({'search_query': item})
                    elif isinstance(item, dict) and 'query' in item and item['query'].strip():
                        valid_queries.append({'search_query': item['query']})
                    elif isinstance(item, dict) and 'search_query' in item and item['search_query'].strip():
                        valid_queries.append(item)
                
                tool_call['follow_up_queries'] = valid_queries
            elif tool_call['follow_up_queries'] is None or (isinstance(tool_call['follow_up_queries'], list) and len(tool_call['follow_up_queries']) == 0):
                tool_call['follow_up_queries'] = []
        else:
            tool_call['follow_up_queries'] = []
        
        feedback = Feedback(**tool_call)
    except Exception as e:
        print(f"处理反馈出错: {str(e)}")
        # 如果解析失败，创建一个默认的通过反馈
    ...
    if feedback.grade == "pass" or state["search_iterations"] >= 2:
        ...
        # 检查是否有有效的后续查询
        if not feedback.follow_up_queries or len(feedback.follow_up_queries) == 0:
            # 如果没有有效查询，强制完成章节
            return Command(
                update={"completed_sections": [section]},
                goto=END
